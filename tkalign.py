...
if __name__=='__main__':
    ...
    import os, pickle, warnings, itertools, re
    ...
    from tkalign_utils import full, values2ranks as ranks, regress as reg, identifiability as ident, count_negs
    ...
    def func(alignfile,subs_inds={'temp': range(0,10), 'test': range(10,20)},nblocks=100,pre_hrc_fwhm=3,post_hrc_fwhm=3):    
        """
        pre_hrc_fwhm: smoothing kernel (mm) for high-res connectomes. Default 3
        post_hrc_fwhm: smoothing kernel after alignment. Default 3
        nblocks: how many (parcel x parcel) blocks to examine
        """
        ...
        '''
        """To find which parcels do well or badly with tkfunc3. Are good parcels also fMRI-reliable parcels? Next thing is to replace the 'scores' with location of blue dot relative to red and green dots (use code similar to plotter). 
        Is the green dot actually a good target? (align both with same). Visualise what tkfunc3 does to node strength (using from_source_vertex) and gyral bias. Try plotting a 'new green' (both unaligned).
        """
        p=hutils.surfplot(plot_dir,mesh=hcp.mesh.inflated,plot_type = 'open_in_browser')
        znmm2 = np.vstack([znmm,znmm])
        parcel_scores_raw=znmm2.ravel()
        parcel_ids_raw = blocks_for_inter.ravel() #would include some duplicates
        parcel_ids=np.unique(parcel_ids_raw)

        parcel_scores = np.zeros(len(parcel_ids),dtype=float)
        for i in range(len(parcel_ids)):
            indices=np.where(parcel_ids_raw==parcel_ids[i])[0]
            average_score = np.nanmean(parcel_scores_raw[indices])
            parcel_scores[i] = average_score
        score_map = np.zeros((align_nparcs))
        for i in range(len(parcel_ids)):
            score_map[parcel_ids[i]] = parcel_scores[i]
        score_map_full = score_map @ align_parc_matrix    
        p.plot(-score_map_full)
        '''
        ...
        if to_plot and ident_grouped_type=='perparcel':
            ...
            def plot_parc_multi(strings):
                for string in strings:
                    plot_parc(eval(string),string)

            if get_similarity_pairwise:
                plot_parc_multi(['mfxomi','fxoMmi'])
            if get_similarity_average:
                plot_parc_multi(['mai','maoi','maroi','anN','arnN'])
    ...
    alignfiles=[    'hcpalign_movie_temp_scaled_orthogonal_20-4-7_TF_0_0_0_FFF_S300_False',
    'hcpalign_movie_temp_optimal_transport_20-4-7_TF_0_0_0_FFF_S300_False',
    'hcpalign_movie_temp_permutation_20-4-7_TF_0_0_0_FFF_S300_False',
    'hcpalign_rest_FC_temp_scaled_orthogonal_20-4-7_TF_0_0_0_FFF_S300_False_FCSchaefer1000',
    'hcpalign_rest_FC_temp_optimal_transport_20-4-7_TF_0_0_0_FFF_S300_False_FCSchaefer1000',
    'hcpalign_movie_temp_optimal_transport_20-4-7_TF_0_0_0_FFF_S200_False',
    'hcpalign_movie_temp_scaled_orthogonal_20-4-7_TF_0_0_0_FFF_S100_False',
    'hcpalign_rest_FC_pair_scaled_orthogonal_20-4-7_TF_0_0_0_FFF_S300_False_FCSchaefer1000',
    'hcpalign_rest_FC_pair_optimal_transport_20-4-7_TF_0_0_0_FFF_S300_False_FCSchaefer1000',
    'hcpalign_movie_pair_scaled_orthogonal_20-4-7_TF_0_0_0_FFF_S100_False',
    'hcpalign_movie_pair_optimal_transport_20-4-7_TF_0_0_0_FFF_S300_False',
    'hcpalign_movie_pair_scaled_orthogonal_20-4-7_TF_0_0_0_FFF_S300_False']

    #alignfiles=alignfiles[0:7]
    alignfiles= alignfiles[-1:][::-1]

    for alignfile in alignfiles:
        for pre_hrc_fwhm in [0,3,5,7]:
            for post_hrc_fwhm in [3]:
                print(f'\nX pre{pre_hrc_fwhm} post{post_hrc_fwhm} {alignfile[9:]}')
                func(alignfile, subs_inds = {'temp': range(0,10), 'test': range(10,20)},nblocks=100,pre_hrc_fwhm=pre_hrc_fwhm,post_hrc_fwhm=post_hrc_fwhm)